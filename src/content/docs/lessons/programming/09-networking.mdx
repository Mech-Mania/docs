...
TCP is the reverse. It requires a digital handshake before sending data, and requires confirmation that a packet has arrived before it will send a new one. On the recieving end, packets are also reconstructed in the order which they were sent. It is much slower and more vulnerable to stalls than UDP, but with it data transfer is reliable and has much lower packet loss.
...
To read or write any UDP data from an ESP32, we use the `NetworkUDP.h` library from Espressif's [Network API](https://docs.espressif.com/projects/arduino-esp32/en/latest/api/network.html#networkudp).
...
To recieve data over UDP, we must first create a listener on the port each datagram is being sent to.
