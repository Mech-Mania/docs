...
:::tip
    While this lesson will go over transmitting data through WiFi on personal computers as well, it will not go over creating a local network through them. You can connect to an ESP32's local network through your personal computer as you would any WiFi network.
:::
...
We can also use our personal computers to act as a UDP/TCP client/server. While setting up a PC as a local network is rather complex, connecting to a network and using it as an medium to communicate through UDP/TCP is simpler.
