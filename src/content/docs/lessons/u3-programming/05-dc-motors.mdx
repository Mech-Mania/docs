...
<Tabs syncKey="Boards">
    ...
    <TabItem label="ESP32">
        ...
        ![MCPWM Units](../../../../assets/lessons/u3-programming/05-02-mcpwm-unit.png)
        
        If we consider the above diagram, we can follow that to individually drive a single pwm lane, for example pwm0a, then we need to index the unit, operator, and lane, as well as assigning a timer (for our purposes driving brushed DC motors, we can assign every motor the same timer).
        
        It is worth mentioning that while multiple timers can be used across PWM output pairs, each individual pair should have a single timer, or else both motor driver inputs may be on at the same time, and short the mosfet.
        ```c++  

        #define JOYSTICK_RESOLUTION 9

        void setMotor(mcpwm_unit_t unit, float val){
            float duty = abs(val) * 100.0f / 255.0f;
            duty = constrain(duty,0.0f,100.0f);

            if (val > 0) {
                mcpwm_set_duty(unit, MCPWM_TIMER_0, MCPWM_GEN_A, duty);
                mcpwm_set_duty_type(unit, MCPWM_TIMER_0, MCPWM_GEN_A, MCPWM_DUTY_MODE_0);

                mcpwm_set_signal_low(unit, MCPWM_TIMER_0, MCPWM_GEN_B); // Ensure other pin is OFF
            } else if (val < 0) {
                mcpwm_set_signal_low(unit, MCPWM_TIMER_0, MCPWM_GEN_A);

                mcpwm_set_duty(unit, MCPWM_TIMER_0, MCPWM_GEN_B, duty);
                mcpwm_set_duty_type(unit, MCPWM_GEN_B, MCPWM_DUTY_MODE_0);
            } else {
                mcpwm_set_signal_low(unit, MCPWM_TIMER_0, MCPWM_GEN_A);
                mcpwm_set_signal_low(unit, MCPWM_TIMER_0, MCPWM_GEN_B);
            }
         }
